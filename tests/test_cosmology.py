...
def test_n_H_tot_positive(cosmo):
    assert n_H_tot(jnp.array(50.0), cosmo) > 0


def test_n_H_tot_scales_as_cubed_redshift(cosmo):
...
def test_n_H_tot_increases_with_redshift(cosmo):
    assert n_H_tot(jnp.array(100.0), cosmo) > n_H_tot(jnp.array(10.0), cosmo)


def test_n_H_tot_depends_on_omega_b(cosmo):
    ...
    cosmo_high_b = cosmology(
        ...
        Omega_b=cosmo.Omega_b * 2, Omega_c=cosmo.Omega_c,
        Y_He=cosmo.Y_He,
    )
